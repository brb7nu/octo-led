...
void filter(AccelerometerDefinition *accelerometer){
	...
	accelerometer->xNormalized = 100 * (accelerometer->xAvg - accelerometer->xMin) / (accelerometer->xMax - accelerometer->xMin);	// generate scaled x value

	accelerometer->yAvg = accelerometer->ySum >> 3;
	accelerometer->yNormalized = 100 * (accelerometer->yAvg - accelerometer->yMin) / (accelerometer->yMax - accelerometer->yMin);	// generate scaled x value

	accelerometer->zAvg = accelerometer->zSum >> 3;
	accelerometer->zNormalized = 100 * (accelerometer->zAvg - accelerometer->zMin) / (accelerometer->zMax - accelerometer->zMin);	// generate scaled x value
